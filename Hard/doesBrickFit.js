...
function doesBrickFit(a,b,c,h,w) {
  if((Math.min(a,b) <= Math.min(h,w) && Math.max(a,b) <= Math.max(h,w)) || (Math.min(a,c) <= Math.min(h,w) && Math.max(a,c) <= Math.max(h,w)) || (Math.min(b,c) <= Math.min(h,w) && Math.max(b,c) <= Math.max(h,w))){
    return true;
  }
  else return false
}
